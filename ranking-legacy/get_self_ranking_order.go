...
/* ACCURACY: fully guessed this one from observing PP20th's behaviour
 * Whatever number returned here is shown in the "Rank" column for the player */
func (commonProtocol *CommonProtocol) getSelfRankingOrder(err error, packet nex.PacketInterface, callID uint32, uniqueID types.UInt32, category types.UInt32, orderParam rankinglegacytypes.RankingOrderParam) (*nex.RMCMessage, *nex.Error) {
	if err != nil {
	...
	data, nexErr := database.GetOwnRanking(commonProtocol.manager, connection.PID(), uniqueID, category, orderParam)
	if nexErr != nil {
	...
	if endpoint.LibraryVersions().Ranking.GreaterOrEqual("2.0.0") {
		rmcResponse.MethodID = rankinglegacy.MethodUnk0xD
	} else {
		rmcResponse.MethodID = rankinglegacy.MethodUnk0xDNEX1
	}
