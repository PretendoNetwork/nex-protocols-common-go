...
func (commonProtocol *CommonProtocol) deleteScore(err error, packet nex.PacketInterface, callID uint32, uniqueID types.UInt32, category types.UInt32) (*nex.RMCMessage, *nex.Error) {
	if err != nil {
	...
	nexErr := database.DeleteScoresByUserAndCategory(commonProtocol.manager, connection.PID(), uniqueID, category)
	if nexErr != nil {
	...
	if endpoint.LibraryVersions().Ranking.GreaterOrEqual("2.0.0") {
		rmcResponse.MethodID = rankinglegacy.MethodDeleteScore
	} else {
		rmcResponse.MethodID = rankinglegacy.MethodDeleteScoreNEX1
	}
