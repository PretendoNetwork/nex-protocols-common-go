...
func (commonProtocol *CommonProtocol) deleteAllScore(err error, packet nex.PacketInterface, callID uint32, uniqueID types.UInt32) (*nex.RMCMessage, *nex.Error) {
	if err != nil {
	...
	nexErr := database.DeleteScoresByUser(commonProtocol.manager, connection.PID(), uniqueID)
	if nexErr != nil {
	...
	if endpoint.LibraryVersions().Ranking.GreaterOrEqual("2.0.0") {
		rmcResponse.MethodID = rankinglegacy.MethodDeleteAllScore
	} else {
		rmcResponse.MethodID = rankinglegacy.MethodDeleteAllScoreNEX1
	}
