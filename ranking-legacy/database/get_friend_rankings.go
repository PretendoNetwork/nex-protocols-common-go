...
func GetFriendRankings(manager *commonglobals.RankingManager, friends []uint32, category types.UInt32, orderParam rankinglegacytypes.RankingOrderParam, offset types.UInt32, length types.UInt8) (types.List[rankinglegacytypes.RankingData], *nex.Error) {
	...
	rows, err := manager.Database.Query(`
		...
		/* $5: Friends */
		WHERE s.category = $1 AND s.deleted = false AND s.owner_pid = ANY($5)
		ORDER BY ord
		OFFSET $6
		LIMIT $7
	`,
