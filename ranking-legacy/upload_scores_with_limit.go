...
func (commonProtocol *CommonProtocol) uploadScoresWithLimit(err error, packet nex.PacketInterface, callID uint32, uniqueID types.UInt32, scores types.List[rankinglegacytypes.RankingScoreWithLimit]) (*nex.RMCMessage, *nex.Error) {
	if err != nil {
	...
	// Discard limits for now - we don't know what it does
	noLimitScores := make(types.List[rankinglegacytypes.RankingScore], 0, len(scores))
	for _, score := range scores {
		noLimitScores = append(noLimitScores, rankinglegacytypes.RankingScore{
			Category: score.Category,
			Score:    score.Score,
			Unknown1: score.Unknown1,
			Unknown2: score.Unknown2,
		})
	}

	nexErr := database.UploadScores(commonProtocol.manager, uniqueID, noLimitScores, connection.PID())
	if nexErr != nil {
	...
	rmcResponse.MethodID = rankinglegacy.MethodUploadScoresWithLimit
