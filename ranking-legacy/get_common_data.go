...
func (commonProtocol *CommonProtocol) getCommonData(err error, packet nex.PacketInterface, callID uint32, uniqueID types.UInt32) (*nex.RMCMessage, *nex.Error) {
	if err != nil {
	...
	data, nexErr := database.GetCommonData(commonProtocol.manager, connection.PID(), uniqueID)
	if nexErr != nil {
		commonglobals.Logger.Error(nexErr.Error())
		return nil, nexErr
	}

	retval := types.NewInt16(20) // TODO: result codes are unknown
	...
	if endpoint.LibraryVersions().Ranking.GreaterOrEqual("2.0.0") {
		rmcResponse.MethodID = rankinglegacy.MethodGetCommonData
	} else {
		rmcResponse.MethodID = rankinglegacy.MethodGetCommonDataNEX1
	}
