...
async def complete(pid: int, **fields) -> None:
	...
	async with helpers.connect(pid) as client:
		await client.complete_post_object(helpers.complete_post_param(**fields))
...
async def complete_error(pid: int, **fields) -> str:
	...
	pytest.fail("DataStoreProtocol::CompletePostObject did not throw an error")
...
async def test_completes_an_upload(unique_data_type, flag):
	...
	data_id = await helpers.prepare_post_object(nex.FRIEND_A, data_type=unique_data_type, flag=flag)
...
@pytest.mark.parametrize("data_id", [
	...
	pytest.param(0xFFFFFFFFFFFFFFFF, id="uint64-max"),
])
...
async def test_completing_an_object_prepared_with_the_older_version(unique_data_type):
	"""
	This method can complete an object which was started with
	PreparePostObjectV1, the two versions can be mixed
	"""
